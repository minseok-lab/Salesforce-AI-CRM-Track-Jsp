...
import board.UserDao;
import board.UserDaoImpl;
import util.WebUtil;
import board.UserVo;
...
public class UserServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if("joinform".equals(actionName)) {
			...
			/*
			RequestDispatcher rd = request.getRequestDispatcher("/WEB-INF/view/user/joinform.jsp");
      ...
      RequestDispatcher rd = request.getRequestDispatcher("/WEB-INF/view/user/joinsuccess.jsp");
      rd.forward(request, response);
...
//      }
      ...
      response.getWriter().write(dao.idCheck(email)); 
			...
			WebUtil.forward(request, response, "/WEB-INF/view/main/index.jsp");
			...
			if(vo==null) {
				...
				response.sendRedirect("/helloweb/user?a=loginform&result=fail");
			} else {
				System.out.println("성공");
				HttpSession session = request.getSession(true);
				session.setAttribute("authUser", vo);
				
				response.sendRedirect("/helloweb/main");
				return;
			}
			...
			WebUtil.redirect(request, response, "/helloweb/main");
			response.sendRedirect("/helloweb/main");
		}
